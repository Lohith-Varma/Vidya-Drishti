...
router.get('/:username', authMiddleware, async (req, res) => {
  try {
    ...
    const stats = await codingStatsService.getHackerRankStats(username)
    res.json(stats)
  } catch (error) {
    res.status(502).json({ message: error.message || 'Failed to fetch HackerRank stats.' })
  }
})

// POST /api/integrations/hackerrank/:username/refresh
router.post('/:username/refresh', authMiddleware, async (req, res) => {
  try {
    ...
    res.status(502).json({ message: error.message || 'Failed to refresh HackerRank stats.' })
  }
})
